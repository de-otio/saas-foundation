 ...
 */
export async function seal(payload: Uint8Array, key: CryptoKey): Promise<string> {
 ...
 */
export async function unseal(token: string, key: CryptoKey): Promise<Uint8Array | null> {
  let envelope: Buffer;
  try {
  ...
  const ciphertext = envelope.subarray(IV_LENGTH_BYTES);
